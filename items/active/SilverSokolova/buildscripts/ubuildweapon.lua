...
require "/items/active/SilverSokolova/buildscripts/uabilities.lua"
...
function build(directory, config, parameters, level, seed)
  ...
  if config.altAbility and config.altAbility.shieldHealth then
    config.altAbility.shieldHealth = config.altAbility.shieldHealth * root.evalFunction("ushieldLevelMultiplier", configParameter("level", 1))
  end
  ...
  parameters.primaryAbility.baseDamageFactor = valueOrRandom(parameters.primaryAbility.baseDamageFactor, seed, "baseDamageFactor")
  ...
  config.primaryAbility.baseDamage = scaleConfig(parameters.primaryAbility.baseDamageFactor, config.primaryAbility.baseDamage)
  ...
  if not config.primaryAbility.projectileParameters then
    ...
    parameters.primaryAbility.baseDamage = util.round((parameters.primaryAbility.baseDps or config.primaryAbility.baseDps or 0) * config.damageLevelMultiplier, 1)
    parameters.primaryAbility.baseDamageFactor = parameters.primaryAbility.baseDamage
  ...
  -- preprocess ranged primary attack config
    ...
    config.primaryAbility.baseDamage = util.round((parameters.primaryAbility.baseDps or config.primaryAbility.baseDps or 0) * (parameters.primaryAbility.fireTime or config.primaryAbility.fireTime or 1.0) * config.damageLevelMultiplier, 1)/3
    --nerf for shotguns
    if config.primaryAbility.projectileCount > 1 then
--	if config.primaryAbility.baseDamage then--and (config.primaryAbility.baseDamage-(config.primaryAbility.projectileCount*1.2) > config.primaryAbility.projectileCount) then
--		config.primaryAbility.baseDamage = config.primaryAbility.baseDamage-(config.primaryAbility.projectileCount*1.2)
--		if config.primaryAbility.baseDamage < 1 then config.primaryAbility.baseDamage = 2 end
--	end
	if config.primaryAbility.energyUsage > 0 then
  ...
  --config.uextraDirectives = parameters.uextraDirectives or config.uextraDirectives or ""
  --config.ubackingDirectives = parameters.ubackingDirectives or config.ubackingDirectives or ""
  ...
  --if config.directives ~= config.ubackingDirectives..config.directives..config.uextraDirectives then
  --config.directives = config.ubackingDirectives..config.directives..config.uextraDirectives end
  --replacePatternInData(config, nil, "<directives>", config.directives)
  end
...
--[[
  ...
  config.tooltipFields.dyeLabel = configParameter("udyeable") and "^gray;(Dyeable)" or ""
...
--  end
  if config.primaryAbility then
	...
	if type(parameters.primaryAbility.projectileType) == "string" then config.tooltipFields.damageKindImage = uassetmissing("/interface/utooltips/"..parameters.primaryAbility.projectileType..".png","/interface/utooltips/uassetmissing.png")
...
--todo: loop
        if #projectiles >= 1 then config.tooltipFields.damageKindImage = uassetmissing("/interface/utooltips/"..projectiles[1]..".png","/interface/utooltips/uassetmissing.png") end
        if #projectiles >= 2 then config.tooltipFields.damageKindBImage = uassetmissing("/interface/utooltips/"..projectiles[2]..".png","/interface/utooltips/uassetmissing.png") end
        if #projectiles >= 3 then config.tooltipFields.damageKindCImage = uassetmissing("/interface/utooltips/"..projectiles[3]..".png","/interface/utooltips/uassetmissing.png") end
        if #projectiles >= 4 then config.tooltipFields.damageKindDImage = uassetmissing("/interface/utooltips/"..projectiles[4]..".png","/interface/utooltips/uassetmissing.png") end
      end
    end
  end
  ...
  if not config.fixedRarity then
    ...
    config.rarity = rarities[root.evalFunction("urarity", math.floor(configParameter("level", 1)))] or config.rarity or "legendary"
  end
  if config.rarity == "essential" then config.tooltipFields.rarityLabel = "Epic" end

  if config.itemName == "ubonehammer" and parameters.primaryAbilityType == "axecleave" then parameters.primaryAbilityType = "uhammer" elseif
  parameters.primaryAbilityType == "axecleave" then parameters.primaryAbilityType = "uaxe" end
