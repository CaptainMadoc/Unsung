...
function realUpdate(dt)
	...
	if data.selected then
		shift = -((3 / data["maxMagazine"]) * (data.selected + 1))
	end
	local aa = 1
	local countedAmmo = 1
	for i,v in pairs(data["magazine"]) do
		for i=1,v.count do
			...
			if v.parameters.fired then
				localAnimator.addDrawable({image = data["uiShell"], position = vec2.add(position, {((4 / data["maxMagazine"]) * countedAmmo) + shift + extraShift, -3.25}), transformation = matt, fullbright = true, color = {255,255 / 2,255 / 2,255}}, "overlay")
			else                                                                                                                                       
				localAnimator.addDrawable({image = data["uiShell"], position = vec2.add(position, {((4 / data["maxMagazine"]) * countedAmmo) + shift + extraShift, -3.25}), transformation = matt, fullbright = true}, "overlay")
			end
		end
	end
